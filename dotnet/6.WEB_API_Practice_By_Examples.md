...
Tạo mới một folder tên là **DataShaping** và thêm mới các class và interface sau:
...
Ta tạo thêm class `LinkResourceBase` như sau, class này sẽ được dùng để chứa tất cả các link của ta.
...
Tạo mới một class `ShapedEntity` trong folder **Models** như sau:
...
`FetchDataForEntity` trong `DataShper` ta modify lại như sau:
...
Trong folder **LinkModels** ta thêm mới một class `LinkResponse` như sau:
...
Trong class `ServiceExtension` ta thêm vào một method `AddCustomMediaTypes` như sau:
...
Trong `ConfigureServices` của `Startup.cs` bên dưới **AddControllers** ta đăng ký thêm service như sau:
...
Ta thêm mới một action filter `ValidateMediaTypeAttribute` bên trong folder **ActionFilters** như sau:
...
Bên trên action `GetEmployeesForCompany` trong controller `EmployeesController` ta modify lại bằng cách ta thêm action filter mà ta vừa thêm vào như sau:
...
Ta tạo mới một folder đặt tên là **Utility** và thêm mới class `EmployeeLinks` như sau:
...
Trong class `Entity` ta modify lại như sau:
...
Trong `ConfigureServices` ta đăng ký thêm DI như sau:

```c#
services.AddScoped<EmployeeLinks>();
```

Trong `EmployeesController:` ta modify lại như sau:
...
```
Trong action `GetEmployeesForCompany` của controller `EmployeesController` ta modify lại như sau:
...
Ta thực hiện kiểm tra lại bằng Postman sẽ được kết quả như sau:

![picture](./images/41.PNG)

# 27. Làm việc với requests OPTIONS và HEAD.

## 27.1 OPTIONS HTTP request.

Request **OPTIONS** cho ta biết loại request nào được allow với URL mà ta đang muốn gọi (get, post, put, delete, options)

Trong CompaniesController ta add thêm hai actions như sau:
...
Ta thực hiện Postman để kiểm tra lần lượt như sau:

![picture](./images/42.PNG)

![picture](./images/43.PNG)

## 27.2 Head HTTP Request

Head request giống như GET nhưng sẽ không có phần Body trả về, nó được dùng để lấy về các thông tin quyền truy cập, validate, và những lần sử đổi gần đây của api ta muốn lấy thông tin.

Trong action `GetEmployeesForCompany` của `EmployeesController` ta sửa đổi như sau:
...
Ta thực hiện kiểm tra lại bằng Postman như sau:
...
Trong một REST API, đôi khi ta cần chỉ cho người dùng làm cách nào để interact với API của ta tạo ra. Để làm được điều đó ta sẽ phải implement **Root Document* hay còn gọi là điểm bắt đầu của API.

Ta cần add name cho 2 actions ở CompaniesController là `GetCompanies` và `CreateCompany`
...
Ta thêm mới một controller tên là `RootController` như sau
...
Trong method `AddCustomMediaTypes` của class `ServiceExtension` ta đăng ký thêm suport media type như sau:
...
`AddCustomMediaTypes` sẽ có dạng như sau:
...
## 29.1 Install package and configurtion

Trong sample này thì ta cần cài đặt package `Microsoft.AspNetCore.Mvc.Versioning` vào dự án của mình để phục vụ việc đánh versioning cho API của ta.

Trong class `ServiceExtensions` mà ta dùng chứa các phương thức mở rộng ta thêm một phương thức mở rộng có tên là `ConfigureVersioning` như sau:
...
* `ReportApiVersions` trả về version của api được gọi cho response.

* `AssumeDefaultVersionWhenUnspecified` trả về version mặc định nếu client không chỉ ra version cho api khi gọi.

* `DefaultApiVersion` dùng để thiết đặt version mặc định cho api.

Trong `ConfigureServices` của **Startup.cs** ta thực hiện thêm mới service mà ta vừa tạo bên trên như sau:

```c#
services.ConfigureVersioning();
```

## 29.2 Versioning example

Ta thêm mới một controller tên là `CompaniesV2Controller` như sau
...
Trong `CompaniesController` ta đánh lại version như sau:
...
### 29.2.1 Using Query String

Khi sử dụng query string API mà ta access sẽ trông như sau

```
https://localhost:5001/api/companies?api-version=2.0
```

Ta dung Postman để test lại API với version mới xem như thế nào.
...
### 29.2.2 Using URL versioning

Ta sửa lại `CompaniesV2Controller` như sau, ta lưu ý rằng từ giờ ta sẽ không gọi versioning bằng query string cho `CompaniesV2Controller` được nữa.
...
Ta sử dụng Postman để kiểm tra lại như sau:

![picture](./images/48.PNG)

### 29.2.3 HTTP header versioning

Trong method `ConfigureVersioning` của class extension ta sửa lại bằng cách thêm dòng lệnh sau `opt.ApiVersionReader = new HeaderApiVersionReader("api-version");`
...
Ta sửa lại controller `CompaniesV2Controller` như sau:
...
Ta kiểm tra lại với Postman như sau:

![picture](./images/49.PNG)

### 29.2.4 Deprecating versions

Nếu như bạn đang muốn ngừng sử dụng một version nào đó của API và bạn không muốn remove nó ra khỏi hệ thống thì bạn có thể làm như sau:

```c#
[ApiVersion("2.0", Deprecated = true)]
```

Ta dùng Postman để kiểm tra lại như sau:

![picture](./images/50.PNG)

### 29.2.5 Using conventions

Nếu như bạn có nhiều version của một controller bạn có thể làm như sau:

Lúc này bạn phải remove atribute **[ApiVersion]** ở các controller tương ứng.
